...
var mcMySql = new MySqlHelper();
mcMySql.conSetting = config.mcSaveMySql;
        ...
        for (var n = 0; n < commonArea.length; n++) {
            ...
            if ((<any[]>rows).length > 0) {
                for (var i = 0; i < (<any[]>rows).length; i++) {
                    ...
                    var rnt = await odb.sqlAsync(conn, sql_1, []);
                }
            }
        }
    } catch (err) {
...
async function queryJZHDDX(areaCode) {
    try {
        var odb = new oracledb();
        var conn = await odb.connect(config.orgOrcale);
...
async function queryResourceList() {
    ...
    var opt = new PostOption(url, '', {
        username: 'DC_API_430900000000',
        password:'kbzxd53r',
    })
    var rnt = await api.getAsync(opt);
    console.log(rnt);
 ...
 * @param rows
 */
async function sendRquerstBatch(areaCode) {
    try {
        var dt = new dtUtil.DateTimeUtil();
...
async function resultDate() {
    ...
    try {
        ...
        var rnt = await odb.queryAsync(conn, sql, []);
        if ((<any[]>rnt).length == 0) {
            console.log("未找到相关数据");
            return;
        }
        ...
        var tableName = await Helplog.queryAsync(sql_2, [commonArea[0]]);
        console.log(tableName);

        for (var n = 0; n < (<any[]>tableName).length; n++) {
            ...
            for (var i = 0; i < (<any[]>rnt).length; i++) {
                ...
                var hdData = await odb.queryAsync(conn, sql_3, []);
                if ((<any[]>hdData).length > 0) {
                    //取key和值
                    for (var key in hdData) {
                        ...
                        await Helplog.queryAsync(sql_4, []);
                    }
                    
                }
                //市州接收协查结果，并将数据状态修改为 45-市州已接收协查结果；
                var sql_6 = "update HD_XCHDPC set BBEX0003='45' where BBEX0001='" + rnt[i].BBEX0001 + "'";
                await odb.sqlAsync(conn, sql_6, []);
                await updateRqurie(rnt[i].AXCP0002, rnt[i].BBEE0002);
 ...
 * 生成报告
 * */
async function generateReport(tableId, tableName, BBEX0001, BBEE0001, BBEE0002) {
    //1,查询表的对应列
 ...
 * 市级反馈委托核对结果
 **/
async function feedBackJZ(param) {
    var odb = new oracledb();
    var conn = await odb.connect(config.orgOrcale);
 ...
 * @param state
 */
async function updateJZWTHDDXState(serial,state) {
    var odb = new oracledb();
    var conn = await odb.connect(config.orgOrcaleCS);
